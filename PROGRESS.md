...
>
> **앱 이름 확정 (2026-09-21)**: 앱 이름을 **쿠피(Coupi)**로 확정했다. 이 저장소가 쿠피 프로젝트의 공식 작업 공간이며, GitHub로 이전 관리를 시작한다.
...
- [ ] LLM 벤더/모델 확정 (`docs/00-개요.md` Open Question #1, `docs/02-쿠폰판별로직.md`)
...
**쿠폰 판별 (CouponClassifierService)**
- [ ] `filter_domains`/`filter_keywords` 테이블 및 초기 시드 데이터 적재 (`docs/02-쿠폰판별로직.md`)
- [ ] 규칙 필터 로직 구현 (도메인 우선 → 키워드 매칭)
- [ ] LLM 프롬프트/구조화 출력 스키마 구현
- [ ] confidence 임계값 정책 구현 (<0.7 폐기, 0.7~0.85 로그, >0.85 정상)
- [ ] `coupon_pattern_cache` 캐싱 로직 구현
- [ ] LLM 실패/타임아웃 재시도 및 `pending_review` 큐 구현 (`docs/08-에러처리및엣지케이스.md`)
...
- [ ] 정밀도/재현율 측정 대시보드 구성 (`docs/02-쿠폰판별로직.md`)
